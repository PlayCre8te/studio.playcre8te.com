...
// ===== Monetization Hub (add-only) ==========================================
// Safe default (won't override if you already set BILLING_API elsewhere)
...
// Auth helpers (use your existing if present)
function __mon_getIdToken() {
  try {
...
const __mon_base = (u) => (u || '').replace(/\/+$/, '');
async function __mon_req(path, opts = {}) {
  const token = __mon_getIdToken();
  const headers = {
    ...(opts.body ? { 'Content-Type': 'application/json' } : {}),
    ...(token ? { 'Authorization': `Bearer ${token}` } : {}),
    ...(opts.headers || {}),
  };
  const url = `${__mon_base(BILLING_API)}/${String(path).replace(/^\/+/, '')}`;
  ...
  if (!res.ok) {
    console.error('Monetization API error', res.status, text);
    throw new Error(`${res.status}`);
  }
  try { return JSON.parse(text); } catch { return text; }
}

// Minimums (match backend)
const __MON_MIN = {
  weekly:       299,
  monthly:      299,
  quarterly:    897,
...
function __mon_mapInterval(val) {
  switch ((val || '').toLowerCase()) {
    case 'weekly':      return { interval: 'week',  interval_count: 1, floorKey: 'weekly' };
    case 'monthly':     return { interval: 'month', interval_count: 1, floorKey: 'monthly' };
    case 'quarterly':   return { interval: 'month', interval_count: 3, floorKey: 'quarterly' };
    case 'semi_annual': return { interval: 'month', interval_count: 6, floorKey: 'semi_annual' };
    case 'annual':      return { interval: 'year',  interval_count: 1, floorKey: 'annual' };
    default: throw new Error('invalid_interval');
  }
}

// Keep a small cache so the Edit modal can prefill and we can refresh quickly
let __mon_cache = { items: [], lastSort: { type: 'all', active: 'all' } };

// ---------- Overlay (idempotent + responsive) ----------
function __mon_injectHubOnce() {
  if (document.getElementById('monHub')) return;

  const html = `
  <div id="monHub" class="fixed inset-0 z-[9998] hidden">
    <!-- Backdrop -->
    <div class="absolute inset-0 bg-black/60"></div>

    <!-- Modal shell: mobile=full-screen sheet; md+=centered card -->
    <div class="absolute inset-0 flex items-end md:items-center justify-center p-0 md:p-4">
      <div
        class="w-full md:max-w-4xl bg-gray-900 text-gray-100 md:rounded-2xl ring-1 ring-white/10 shadow-xl
               flex flex-col
               h-[100dvh] md:h-auto md:max-h-[90dvh] md:min-h-[60dvh]">

        <!-- Header (sticky) -->
        <div class="sticky top-0 z-10 bg-gray-900 border-b border-white/10 px-4 py-3 flex items-center justify-between">
          <div class="flex items-center gap-3">
            <div class="text-lg font-semibold">Monetization</div>
            <div id="monBrandBadge" class="text-sm text-gray-400"></div>
          </div>
          <button id="monHubClose" class="text-gray-400 hover:text-white" aria-label="Close">✕</button>
        </div>

        <!-- Tabs (sticky under header) -->
        <div class="sticky top-[52px] md:top-[52px] z-10 bg-gray-900 px-4 pt-3 pb-2 flex items-center gap-2 border-b border-white/10">
          <button id="monTabProducts" class="px-3 py-1.5 rounded bg-gray-800 hover:bg-gray-700">Products</button>
          <button id="monTabCreate"   class="px-3 py-1.5 rounded hover:bg-gray-800">Create</button>
          <div class="ml-auto text-xs sm:text-sm text-gray-400" id="monAcctHint"></div>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          ...
          <!-- PRODUCTS LIST -->
          <div id="monProductsPane">
            <div class="text-xs sm:text-sm text-gray-400">
              Toggle <span class="text-gray-200 font-medium">Active</span> to decide what shows on your media network.
              For subscriptions, only one plan per interval (weekly, monthly, quarterly, semi-annual, annual) can be active at a time.
              One-time and rentals can have many.
            </div>

            <!-- Filters (existing sorting UI from your last version) -->
            <div class="mt-2 flex flex-wrap items-center gap-2">
              <div class="text-xs text-gray-400">Sort by:</div>
              <select id="monSortType" class="px-2 py-1.5 bg-gray-800 rounded border border-white/10 text-sm">
                <option value="all">All types</option>
                <option value="one_time">One-time</option>
                <option value="rental">Rental</option>
                <option value="subscription_weekly">Subscription — Weekly</option>
                ...
                <option value="subscription_semi_annual">Subscription — Semi-Annual</option>
                <option value="subscription_annual">Subscription — Annual</option>
              </select>
              <select id="monSortActive" class="px-2 py-1.5 bg-gray-800 rounded border border-white/10 text-sm">
                <option value="all">Active + Inactive</option>
                <option value="active">Active only</option>
                <option value="inactive">Inactive only</option>
              </select>
              <button id="monResetSort" class="px-2 py-1.5 bg-gray-800 hover:bg-gray-700 rounded text-sm">Reset</button>
            </div>

            <!-- Table with its own scroll area + sticky header -->
            <div class="mt-2 border border-white/10 rounded-lg overflow-hidden">
              <div class="max-h-[55vh] md:max-h-[50vh] overflow-auto">
                <table class="w-full text-xs sm:text-sm">
                  <thead class="text-gray-400 bg-gray-900 sticky top-0 z-10 border-b border-white/10">
                    <tr>
                      <th class="py-2 px-3 text-left">Name</th>
                      <th class="py-2 px-3 text-left">Type</th>
                      <th class="py-2 px-3 text-left">Interval</th>
                      <th class="py-2 px-3 text-left">Price</th>
                      <th class="py-2 px-3 text-left">Active</th>
                    </tr>
                  </thead>
                  <tbody id="monProductsBody" class="divide-y divide-white/10"></tbody>
                </table>
              </div>
            </div>

            <div id="monProductsEmpty" class="hidden text-sm text-gray-300 mt-3">
              No products yet. Create one in the <span class="text-blue-300">Create</span> tab.
            </div>
          </div>

          <!-- CREATE FORM -->
          <div id="monCreatePane" class="hidden">
            <form id="monCreateForm" class="space-y-3">
              <input type="hidden" id="monBrandId" value="">
              <div>
                <label class="block text-sm mb-1">Product name</label>
                <input id="monName" type="text" class="w-full px-3 py-2 bg-gray-800 rounded-lg" placeholder="e.g. Monthly Access *REQUIRED" required>
              </div>
              <div>
                <label class="block text-sm mb-1">Description</label>
                <textarea id="monDesc" rows="2" class="w-full px-3 py-2 bg-gray-800 rounded-lg" placeholder="What’s included? *REQUIRED"></textarea>
              </div>
              <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label class="block text-sm mb-1">Type</label>
                  <select id="monType" class="w-full px-3 py-2 bg-gray-800 rounded-lg">
                    <option value="subscription">Subscription</option>
                    <option value="one_time">One-time</option>
                    <option value="rental">Rental</option>
                  </select>
                </div>
                <div id="monIntervalWrap">
                  <label class="block text-sm mb-1">Interval</label>
                  <select id="monInterval" class="w-full px-3 py-2 bg-gray-800 rounded-lg">
                    <option value="monthly">Monthly (min $2.99)</option>
                    <option value="weekly">Weekly (min $2.99)</option>
                    <option value="quarterly">Quarterly (min $8.97)</option>
                    <option value="semi_annual">Semi-Annual (min $17.94)</option>
                    <option value="annual">Annual (min $35.88)</option>
                  </select>
                </div>
                <div>
                  <label class="block text-sm mb-1">Price (USD)</label>
                  <input id="monAmount" type="number" min="2.99" step="0.01" class="w-full px-3 py-2 bg-gray-800 rounded-lg" placeholder="2.99" required>
                  <div id="monPriceHint" class="text-xs text-gray-400 mt-1"></div>
                </div>
              </div>

              <div class="pt-2 flex items-center justify-end gap-2 sticky bottom-0 bg-gray-900">
                <button type="button" id="monCancel" class="px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600">Close</button>
                <button id="monSubmit" class="px-3 py-1.5 rounded bg-indigo-600 hover:bg-indigo-500">Create</button>
              </div>
              <div id="monMsg" class="text-sm text-amber-300"></div>
            </form>
          </div>
  ...
  <!-- EDIT PRODUCT MODAL (new, idempotent) -->
  <div id="monEditModal" class="fixed inset-0 z-[10000] hidden">
    <div class="absolute inset-0 bg-black/60"></div>
    <div class="absolute inset-0 flex items-end md:items-center justify-center p-0 md:p-4">
      <div class="w-full md:max-w-md bg-gray-900 text-gray-100 md:rounded-2xl ring-1 ring-white/10 shadow-xl
                  h-[75dvh] md:h-auto md:max-h-[80dvh] flex flex-col">
        <div class="px-4 py-3 border-b border-white/10 flex items-center justify-between">
          <h3 class="text-lg font-semibold">Edit Product</h3>
          <button id="monEditClose" class="text-gray-400 hover:text-white" aria-label="Close">✕</button>
        </div>
        <form id="monEditForm" class="px-4 py-3 space-y-3 overflow-y-auto">
          <input type="hidden" id="monEditPriceId" value="">
          <div>
            <label class="block text-sm mb-1">Name</label>
            <input id="monEditName" type="text" class="w-full px-3 py-2 bg-gray-800 rounded-lg" required>
          </div>
          <div>
            <label class="block text-sm mb-1">Description</label>
            <textarea id="monEditDesc" rows="3" class="w-full px-3 py-2 bg-gray-800 rounded-lg"></textarea>
          </div>
          <div class="pt-2 flex items-center justify-end gap-2 sticky bottom-0 bg-gray-900">
            <button type="button" id="monEditCancel" class="px-3 py-1.5 rounded bg-gray-700 hover:bg-gray-600">Cancel</button>
            <button id="monEditSave" class="px-3 py-1.5 rounded bg-indigo-600 hover:bg-indigo-500">Save</button>
          </div>
          <div id="monEditMsg" class="text-sm text-amber-300"></div>
        </form>
      </div>
    </div>
  </div>
  ...
  // Tab switching
  const toProducts = () => {
    show($('monProductsPane')); hide($('monCreatePane'));
    $('monTabProducts')?.classList.add('bg-gray-800');
    $('monTabCreate')?.classList.remove('bg-gray-800');
  };
  const toCreate = () => {
    show($('monCreatePane')); hide($('monProductsPane'));
    $('monTabCreate')?.classList.add('bg-gray-800');
    $('monTabProducts')?.classList.remove('bg-gray-800');
  };

  $('monTabProducts')?.addEventListener('click', toProducts);
  $('monTabCreate')?.addEventListener('click', toCreate);

  // Open/close helpers (lock background scroll on open)
  function lockScroll() { try { document.documentElement.style.overflow = 'hidden'; document.body.style.overflow = 'hidden'; } catch {} }
  function unlockScroll() { try { document.documentElement.style.overflow = ''; document.body.style.overflow = ''; } catch {} }
  function closeModal() {
    $('monHub')?.classList.add('hidden');
    unlockScroll();
  }

  $('monHubClose')?.addEventListener('click', closeModal);
  $('monCancel')?.addEventListener('click', closeModal);

  // Close on ESC
  document.addEventListener('keydown', (e) => {
    const visible = !$('monHub')?.classList.contains('hidden');
    const editVisible = !$('monEditModal')?.classList.contains('hidden');
    if ((visible || editVisible) && e.key === 'Escape') {
      if (editVisible) $('monEditModal')?.classList.add('hidden');
      else closeModal();
    }
  });

  // Interval hint toggle
  const typeSel = $('monType'), ivalSel = $('monInterval'), ivalWrap = $('monIntervalWrap'), hint = $('monPriceHint');
  function updateHint() {
    const t = typeSel.value;
    if (t === 'subscription') {
      ivalWrap.classList.remove('hidden');
      const floorKey = __mon_mapInterval(ivalSel.value).floorKey;
      hint.textContent = `Minimum for ${ivalSel.value.replace('_','-')}: $${(__MON_MIN[floorKey]/100).toFixed(2)}.`;
    } else {
      ivalWrap.classList.add('hidden');
      const min = t === 'rental' ? __MON_MIN.rental : __MON_MIN.one_time;
      hint.textContent = `Minimum: $${(min/100).toFixed(2)}.`;
    }
  }
  typeSel?.addEventListener('change', updateHint);
  ivalSel?.addEventListener('change', updateHint);
  updateHint();

  // Create submit
  $('monCreateForm')?.addEventListener('submit', __mon_onCreateSubmit);

  // Edit modal close/cancel
  const closeEdit = () => $('monEditModal')?.classList.add('hidden');
  $('monEditClose')?.addEventListener('click', closeEdit);
  $('monEditCancel')?.addEventListener('click', closeEdit);

  // Edit submit
  $('monEditForm')?.addEventListener('submit', __mon_onEditSubmit);

  // Expose a tiny helper so your opener can lock scroll when showing
  window.__openMonHubAndLock = () => { show($('monHub')); lockScroll(); };
}

// ---------- Data helpers ----------
async function __mon_fetchDefaultAccountId() {
  const sub = __mon_getSub(); if (!sub) return '';
  const data = await __mon_req(`/creators/${encodeURIComponent(sub)}/accounts`);
  const arr = Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []);
  return arr[0]?.accountId || arr[0]?.stripeAccountId || '';
}

async function __mon_fetchProducts() {
  const sub = __mon_getSub(); if (!sub) return [];
  const data = await __mon_req(`/billing/products?creatorId=${encodeURIComponent(sub)}`);
  return Array.isArray(data?.items) ? data.items : (Array.isArray(data) ? data : []);
}

// ---------- Toggle handler (fixed) ----------
async function __mon_toggleActive(e) {
  const input = e?.currentTarget || e?.target || null;
  if (!input) return;

  const priceId = input.getAttribute('data-price-id') || input.dataset?.priceId || '';
  if (!priceId) {
    console.error('toggle: missing data-price-id on input');
    return;
  }
  const creatorId = __mon_getSub();
  const desired = !!input.checked;

  input.disabled = true;
  try {
    await __mon_req(`/billing/products/${encodeURIComponent(priceId)}/active`, {
      method: 'PATCH',
      body: { creatorId, playActive: desired }
    });
    // Refresh so “single active per interval” is reflected immediately
    await __mon_renderProducts();
  } catch (err) {
...
// ---------- Edit helpers (NEW) ----------
function __mon_openEdit(priceId) {
  const $ = (id) => document.getElementById(id);
  const item = (__mon_cache.items || []).find(p => p.priceId === priceId);
  if (!item) return;

  $('monEditPriceId').value = item.priceId || '';
  $('monEditName').value    = item.name || '';
  $('monEditDesc').value    = item.description || '';

  $('monEditMsg').textContent = '';
  $('monEditModal')?.classList.remove('hidden');
}

async function __mon_onEditSubmit(e) {
  e.preventDefault();
  const $ = (id) => document.getElementById(id);
  const creatorId   = __mon_getSub();
  const priceId     = $('monEditPriceId').value;
  const name        = $('monEditName').value.trim();
  const description = $('monEditDesc').value.trim();
  const msgEl       = $('monEditMsg');
  const btn         = $('monEditSave');

  if (!creatorId || !priceId) {
    msgEl.textContent = 'Missing required info.'; 
    return;
  }
  if (!name) {
  ...
  try {
    // Backend expectation: PATCH /billing/products/{priceId}/meta
    await __mon_req(`/billing/products/${encodeURIComponent(priceId)}/meta`, {
      method: 'PATCH',
      body: { creatorId, name, description }
    });

    // Update local cache, then re-render
    const idx = (__mon_cache.items || []).findIndex(p => p.priceId === priceId);
    if (idx >= 0) {
      __mon_cache.items[idx].name = name;
      __mon_cache.items[idx].description = description;
    }
    await __mon_renderProducts();
    // Close
    document.getElementById('monEditModal')?.classList.add('hidden');
  } catch (err) {
    console.error('edit meta failed', err);
...
// ---------- Renderers ----------
function __mon_fmtCents(c) { return `$${(Number(c||0)/100).toFixed(2)}`; }
function __mon_badge(t) {
  const cls = 'px-2 py-0.5 rounded text-xs';
  if (t === 'subscription') return `<span class="${cls} bg-indigo-700/50 ring-1 ring-indigo-400/30">subscription</span>`;
  if (t === 'rental')       return `<span class="${cls} bg-amber-700/40 ring-1 ring-amber-400/30">rental</span>`;
  return `<span class="${cls} bg-emerald-700/40 ring-1 ring-emerald-400/30">one-time</span>`;
}

function __mon_row(p) {
  const ival = (p.type === 'subscription') ? (p.interval || p.planIntervalKey || '') : '';
  return `
    <tr class="border-b border-white/10">
      <td class="py-2 pr-3">
        <div class="flex items-center gap-2">
          <span>${escapeHtml(p.name || '')}</span>
          <button class="mon-edit px-2 py-0.5 text-xs bg-gray-700 hover:bg-gray-600 rounded"
                  data-price-id="${escapeHtml(p.priceId)}"
                  title="Edit name & description">✎ Edit</button>
        </div>
      </td>
      <td class="py-2 pr-3">${__mon_badge(p.type)}</td>
      <td class="py-2 pr-3">${ival ? escapeHtml(ival) : '—'}</td>
      <td class="py-2 pr-3">${__mon_fmtCents(p.amount)} ${escapeHtml(p.currency || 'usd').toUpperCase()}</td>
      <td class="py-2 pr-3">
        <label class="inline-flex items-center gap-2 cursor-pointer select-none">
          <input type="checkbox" class="mon-toggle" data-price-id="${escapeHtml(p.priceId)}" ${p.playActive ? 'checked' : ''}>
          <span>${p.playActive ? 'Active' : 'Inactive'}</span>
        </label>
      </td>
    </tr>`;
}

// Apply current sort filters to a list (helper)
function __mon_applySort(items) {
  const byType = __mon_cache.lastSort?.type || 'all';
  const byActive = __mon_cache.lastSort?.active || 'all';

  let filtered = items.slice();

  if (byType !== 'all') {
    if (byType.startsWith('subscription_')) {
      const want = byType.replace('subscription_', '');
      filtered = filtered.filter(p => p.type === 'subscription' && (p.interval || p.planIntervalKey) === want);
    } else {
      filtered = filtered.filter(p => p.type === byType);
    }
  }

  if (byActive === 'active')   filtered = filtered.filter(p => p.playActive === true);
  if (byActive === 'inactive') filtered = filtered.filter(p => !p.playActive);

  // Keep your “active first” preference inside the filtered set
  filtered.sort((a,b) => (b.playActive === true) - (a.playActive === true));
  return filtered;
}

async function __mon_renderProducts() {
  const body  = document.getElementById('monProductsBody');
  const empty = document.getElementById('monProductsEmpty');
  if (!body) return;
  body.innerHTML = '';

  try {
    // 1) Fetch ALL creator products (unchanged API)
    const all = await __mon_fetchProducts();
    __mon_cache.items = Array.isArray(all) ? all : [];

    // 2) Filter by the brand for which the monetization hub was opened
    //    (brandId is set into the hidden input when openMonetizationHub(brandId) is called)
    const brandId = (document.getElementById('monBrandId')?.value || '').trim();

    // If a brandId is supplied, show ONLY products that belong to that brand.
    // If brandId is empty (unexpected for your flow), fall back to showing nothing
    // to avoid cross-brand leakage.
    let scoped = [];
    if (brandId) {
      scoped = __mon_cache.items.filter(p => (p.brandId || '') === brandId);
    } else {
      scoped = []; // safest default
    }

    // 3) Apply your existing sort/filter UI on the *scoped* list
    const display = __mon_applySort(scoped);
    ...
    body.innerHTML = display.map(__mon_row).join('');

    // 5) Wire toggles + edit buttons within the newly rendered rows
    body.querySelectorAll('input.mon-toggle[data-price-id]').forEach(chk => {
      chk.addEventListener('change', __mon_toggleActive);
    });

    body.querySelectorAll('button.mon-edit[data-price-id]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        const pid = e.currentTarget.getAttribute('data-price-id');
        if (pid) __mon_openEdit(pid);
      });
    });
  } catch (e) {
...
// ---------- Open Hub ----------
async function openMonetizationHub(brandId, brandName) {
  __mon_injectHubOnce();
  const $ = (id) => document.getElementById(id);

  $('monHub')?.classList.remove('hidden');
  $('monBrandId').value = brandId || '';

  // Brand label in header (consistent with Media Management UI)
  const b = (typeof getBrandById === 'function') ? getBrandById(brandId) : null;
  const name = b?.displayName || b?.slug || brandName || brandId || '';
  $('monBrandBadge').textContent = name ? `Brand: ${name}` : '';

  // Account hint (quiet: do not show account ids)
  $('monAcctHint').textContent = '';
  try {
    // Keep the check so flows depending on it still happen,
    // but don't render anything to the UI.
    await __mon_fetchDefaultAccountId();
  } catch {
    // leave hint empty
  }

  // Start on Products tab + refresh
  document.getElementById('monTabProducts')?.click();

  // Hook sort controls (remember choice)
  const typeSel = document.getElementById('monSortType');
  const actSel  = document.getElementById('monSortActive');
  const resetBtn= document.getElementById('monResetSort');

  const refreshWithSort = async () => {
    __mon_cache.lastSort = { type: typeSel.value, active: actSel.value };
    await __mon_renderProducts();
  };

  typeSel?.addEventListener('change', refreshWithSort);
  actSel?.addEventListener('change', refreshWithSort);
  resetBtn?.addEventListener('click', async () => {
    typeSel.value = 'all'; actSel.value = 'all';
    __mon_cache.lastSort = { type: 'all', active: 'all' };
    await __mon_renderProducts();
  });
...
// ---------- Create handler ----------
async function __mon_onCreateSubmit(e) {
  e.preventDefault();
  const $ = (id) => document.getElementById(id);
  const sub = __mon_getSub();
  if (!sub) { $('monMsg').textContent = 'You must be signed in.'; return; }

  // Ensure creator has a Connect account
  let accountId = '';
  try { accountId = await __mon_fetchDefaultAccountId(); } catch {}
  if (!accountId) { $('monMsg').textContent = 'Onboard a Connect account first (Earnings).'; return; }

  const brandId = $('monBrandId').value || '';
  const name = $('monName').value.trim();
  const description = $('monDesc').value.trim();
  const type = $('monType').value;
  const amountUsd = Number($('monAmount').value || '0');

  // price floor validation
  let cents = Math.round(amountUsd * 100);
  let body = {
    creatorId: sub,
    creatorStripeAccountId: accountId,
    brandId, name, description,
    type, amount: cents, currency: 'usd'
  };
  if (type === 'subscription') {
    const ival = $('monInterval').value;
    try {
      const { floorKey } = __mon_mapInterval(ival);
      if (cents < __MON_MIN[floorKey]) {
        $('monMsg').textContent = `Price too low. Minimum for ${ival.replace('_','-')} is $${(__MON_MIN[floorKey]/100).toFixed(2)}.`;
        return;
      }
    } catch { $('monMsg').textContent = 'Choose a valid interval.'; return; }
    body.interval = ival;
  } else {
    const min = (type === 'rental') ? __MON_MIN.rental : __MON_MIN.one_time;
    if (cents < min) {
      $('monMsg').textContent = `Price too low. Minimum is $${(min/100).toFixed(2)}.`;
      return;
    }
  }

  const btn = $('monSubmit'); btn.disabled = true; btn.textContent = 'Creating…'; $('monMsg').textContent = '';
  try {
    const res = await __mon_req('/billing/products', { method: 'POST', body });
    if (res?.priceId) {
      $('monMsg').innerHTML = `✅ Created <span class="font-mono">${res.priceId}</span>`;
      // Jump back to Products and refresh
      document.getElementById('monTabProducts')?.click();
      await __mon_renderProducts();
      // Optionally clear the form
      $('monName').value = ''; $('monDesc').value = ''; $('monAmount').value = '';
    } else {
      $('monMsg').textContent = 'Created, but response missing IDs.';
    }
  } catch (err) {
    console.error(err);
    $('monMsg').textContent = 'Failed to create product.';
  } finally {
  ...
  // === Monetization helpers (additive) =========================================
  function tmSelectedProductIds(formEl) {
    if (!formEl) return { oneId: null, renId: null };
    const oneSel = formEl.querySelector('input[type="radio"][name="tmOneTime"]:checked');
    const renSel = formEl.querySelector('input[type="radio"][name="tmRental"]:checked');
    return {
      oneId: oneSel ? String(oneSel.value) : null,
      renId: renSel ? String(renSel.value) : null
    };
  }

  // Mirrors selection into pricing.one_time/rental.productID without touching pricing.type
  function tmAttachProductPointersToPricing(payload, formEl) {
    if (!payload || !formEl) return payload;
    const { oneId, renId } = tmSelectedProductIds(formEl);
    payload.pricing = payload.pricing || {};
    if (oneId) {
      payload.pricing.one_time = payload.pricing.one_time || {};
      payload.pricing.one_time.productID = oneId;
    }
    if (renId) {
      payload.pricing.rental = payload.pricing.rental || {};
      payload.pricing.rental.productID = renId;
    }
    return payload;
  }

// ───────────────── Monetization selector (state + helpers) ─────────────────
const __tmMonetState = {
  oneTimeProducts: [],          // [{productId,type,name,amount,active,playActive,...}]
  rentalProducts: [],           // same
  selectedOneTimeProductId: null,
  selectedRentalProductId: null,
};

// Safely get creatorSub (adjust if you already have it in memory)
async function tmGetCreatorSub() {
  try {
    if (typeof getUserInfo === 'function') {
      const u = await getUserInfo();                   // your existing /userinfo fetch
      return u?.userid || u?.sub || u?.creatorSub || null;
    }
  } catch {}
  // Fallback: parse from your id_token if you store it
  try {
    const raw = (window.localStorage && localStorage.getItem('id_token')) || '';
    if (raw) {
...
// 1) Fetch active one_time + rental products for this creator
// 1) Fetch active one_time + rental products for this creator (tolerant fields)
async function tmFetchActiveProductsForCreator(creatorSub) {
  if (!creatorSub) return [];
  const base = (typeof BILLING_API === 'string' && BILLING_API) || '';
  const url  = `${base.replace(/\/$/,'')}/billing/products?creatorSub=${encodeURIComponent(creatorSub)}&types=one_time,rental&active=true&playActive=true`;

  // normalize a raw product to the shape your renderer expects
  const normalize = (p) => {
    if (!p) return null;

    const productId = p.productId || p.id || p.product_id || p.stripeProductId;
    if (!productId) return null;

    let type = String(p.type || p.productType || '').toLowerCase().replace(/[\s\-]/g,'_');
    if (type === 'purchase' || type === 'one_time_purchase') type = 'one_time';

    const amount =
      (typeof p.amount === 'number') ? p.amount :
      (typeof p.unit_amount === 'number') ? p.unit_amount :
      (typeof p.unitAmount === 'number') ? p.unitAmount : 0;

    const active     = (p.active === true || p.isActive === true);
    const playActive = (p.playActive === true || p.play_active === true || p.enabled === true);

    // 🔹 BRAND ID (new): pull from common places
    const brandId =
      p.brandId || p.brandID || p.brand ||
      (p.metadata && (p.metadata.brandId || p.metadata.brandID)) || '';

    return {
      productId: String(productId),
      type,
      name: String(p.name || p.displayName || p.label || ''),
      amount: Number(amount || 0),
      currency: (p.currency || 'usd'),
      active,
      playActive,
      updatedAt: p.updatedAt || null,
      brandId: String(brandId || ''),     // ← new
    };
  };


  // try server-filtered first
  try {
    ...
    if (res && res.ok) {
      ...
      return items
        ...
        .filter(p => (p.active && p.playActive && (p.type === 'one_time' || p.type === 'rental')));
    }
  } catch {}

  // fallback: unfiltered → client filter
  try {
    const res = await fetch(`${base.replace(/\/$/,'')}/billing/products?creatorSub=${encodeURIComponent(creatorSub)}`, { credentials: 'include' }).catch(()=>null);
    if (res && res.ok) {
      const json  = await res.json().catch(()=>({}));
      const items = Array.isArray(json?.items) ? json.items : (Array.isArray(json) ? json : []);
      return items
  ...
  // last fallback: window.api.listProducts (if you expose it)
  try {
    if (window.api?.listProducts) {
      const payload = await window.api.listProducts({ creatorSub });
...
// Format $ from cents
function tmFmt(amountCents) {
  const n = Number(amountCents || 0);
  return `$${(n/100).toFixed(2)}`;
}

// 2) Paint radio lists inside the given form
function tmRenderMonetSelectors(formEl, { selectedOneTime, selectedRental, warnIfMissing=false } = {}) {
  if (!formEl) return;
  const oneWrap = formEl.querySelector('#oneTimeList');
  const oneEmpty= formEl.querySelector('#oneTimeEmpty');
  const renWrap = formEl.querySelector('#rentalList');
  const renEmpty= formEl.querySelector('#rentalEmpty');

  // Clear holders
  if (oneWrap)  oneWrap.innerHTML = '';
  if (renWrap)  renWrap.innerHTML = '';

  // One-time
  const one = __tmMonetState.oneTimeProducts;
  if (oneWrap && one?.length) {
    one.forEach(p => {
      const lbl = document.createElement('label');
      lbl.className = 'flex items-center gap-2 text-sm text-gray-200';
      lbl.innerHTML = `
        <input type="radio" name="tmOneTime" value="${p.productId}">
        <span>${p.name || 'One-Time'} — ${tmFmt(p.amount)}</span>
      `;
      oneWrap.appendChild(lbl);
    });
    if (selectedOneTime) {
      const radio = oneWrap.querySelector(`input[type="radio"][name="tmOneTime"][value="${selectedOneTime}"]`);
      if (radio) radio.checked = true;
      else if (warnIfMissing) {
        const warn = document.createElement('div');
        warn.className = 'text-xs text-amber-400 mt-1';
        warn.textContent = 'Previously selected one-time product is no longer available.';
        oneWrap.parentElement.appendChild(warn);
      }
    }
    if (oneEmpty) oneEmpty.classList.add('hidden');
  } else {
    if (oneEmpty) oneEmpty.classList.remove('hidden');
  }

  // Rental
  const ren = __tmMonetState.rentalProducts;
  if (renWrap && ren?.length) {
    ren.forEach(p => {
      const lbl = document.createElement('label');
      lbl.className = 'flex items-center gap-2 text-sm text-gray-200';
      lbl.innerHTML = `
        <input type="radio" name="tmRental" value="${p.productId}">
        <span>${p.name || 'Rental'} — ${tmFmt(p.amount)}</span>
      `;
      renWrap.appendChild(lbl);
    });
    if (selectedRental) {
      const radio = renWrap.querySelector(`input[type="radio"][name="tmRental"][value="${selectedRental}"]`);
      if (radio) radio.checked = true;
      else if (warnIfMissing) {
        const warn = document.createElement('div');
        warn.className = 'text-xs text-amber-400 mt-1';
        warn.textContent = 'Previously selected rental product is no longer available.';
        renWrap.parentElement.appendChild(warn);
      }
    }
    if (renEmpty) renEmpty.classList.add('hidden');
  } else {
    if (renEmpty) renEmpty.classList.remove('hidden');
  }
}

// 3) Merge selection into Title payload (kept + additive)
function tmAttachMonetizationToPayload(payload, formEl) {
  if (!payload || !formEl) return payload;

  const { oneId, renId } = tmSelectedProductIds(formEl);

  // Legacy/Existing: keep writing purchases/rentals (unchanged)
  if (oneId) {
    payload.purchases = payload.purchases || {};
    payload.purchases.one_time = { productID: oneId };
  } else if (payload?.purchases?.one_time) {
    delete payload.purchases.one_time; // selection optional
  }

  if (renId) {
    payload.rentals = payload.rentals || {};
    payload.rentals.rental = { productID: renId };
  } else if (payload?.rentals?.rental) {
    delete payload.rentals.rental; // selection optional
  }

  // NEW (additive): also mirror into pricing.* without touching pricing.type
  payload.pricing = payload.pricing || {};
  if (oneId) {
    payload.pricing.one_time = payload.pricing.one_time || {};
    payload.pricing.one_time.productID = oneId;
...
// 4) Optional: trigger feed build after save (non-blocking)
async function tmTriggerFeedBuild(brandId, titleId) {
  try {
    const base = (typeof FEEDS_API === 'string' && FEEDS_API) || '';
    if (!base) return;
    const url = `${base.replace(/\/$/,'')}/rebuild?brandId=${encodeURIComponent(brandId)}&titleId=${encodeURIComponent(titleId)}`;
    await fetch(url, { method: 'POST', credentials: 'include' });
  } catch (e) {
    console.warn('[FeedBuilder] trigger failed (non-blocking)', e);
  }
}

function tmSyncTogglesWithAvailability(formEl) {
  // If no active one-time products, turn Purchase OFF and disable the toggle
  const hasOne = (__tmMonetState.oneTimeProducts || []).length > 0;
  const chkPur = formEl.querySelector('#mPurchase');
  if (chkPur) {
    if (!hasOne) { chkPur.checked = false; chkPur.disabled = true; }
    else { chkPur.disabled = false; }
  }

  // If no active rental products, turn Rental OFF and disable the toggle
  const hasRen = (__tmMonetState.rentalProducts || []).length > 0;
  const chkRen = formEl.querySelector('#mRental');
  if (chkRen) {
    if (!hasRen) { chkRen.checked = false; chkRen.disabled = true; }
    else { chkRen.disabled = false; }
  }

  // Also clear any stale radio selection if its group is empty
  if (!hasOne) formEl.querySelectorAll('input[type="radio"][name="tmOneTime"]').forEach(n => n.checked = false);
  if (!hasRen) formEl.querySelectorAll('input[type="radio"][name="tmRental"]').forEach(n => n.checked = false);
}
...
const platforms = `
  <div class="space-y-6">
    <p class="text-gray-300">Configure Platform Settings.</p>

    <!-- Platform toggles -->
    <div class="hidden rounded-xl ring-1 ring-white/10 p-4 space-y-4">
    ...
    <div class="mt-4 rounded-xl ring-1 ring-white/10 p-4 space-y-2">
      ...
      <div class="mt-2">
        <select id="beVisibilityLevel" class="w-full max-w-xs p-2 bg-gray-800 rounded border border-white/10">
          <option value="public"${vis==='public'?' selected':''}>Public (Any Visitor)</option>
          <option value="subscribers"${vis==='subscribers'?' selected':''}>All Subscribers</option>
          <option value="private"${vis==='private'?' selected':''}>Private (Paid Subscribers)</option>
        </select>
      </div>
    </div>
  `;
...
(function (global) {
  ...
  function nameOf(it){
  return String(it.title || it.fileName || it.name || it.displayName || it.originalFileName || it.filename || it.label || '');
}

function dateOf(it){
  const mu = it.mediaUrl || it.media_url || {};
  const candidates = [
    it.updatedAt, it.modifiedAt, it.lastModified,
    it.createdAt, it.ingestedAt, it.uploadedAt,
    mu.updatedAt, mu.createdAt, mu.uploadedAt
  ];
...
function sortList(list, mode){
  ...
  switch (mode) {
    case 'oldest':
      arr.sort((a,b)=> dateOf(a) - dateOf(b)); break;
    case 'az':
      arr.sort((a,b)=> nameOf(a).localeCompare(nameOf(b))); break;
    case 'za':
      arr.sort((a,b)=> nameOf(b).localeCompare(nameOf(a))); break;
    case 'latest':
    default:
      arr.sort((a,b)=> dateOf(b) - dateOf(a)); break;
  }
  return arr;
}
  ...
  global.showInlineMediaPicker = function showInlineMediaPicker(items, opts = {}) {
    ...
    el.innerHTML = [
      ...
      '      <div class="flex items-center gap-2">',
      '        <label class="text-[11px] text-gray-400">Sort</label>',
      '        <select id="ipSort" class="bg-gray-800 border border-white/10 text-sm rounded px-2 py-1 text-gray-100">',
    ...
    let sortMode = 'latest';
    let all = sortList(view, sortMode);   // default Latest → Oldest
    let current = all.slice();

    function render(list) {
      const toDraw = sortList(list, sortMode);
      if (!toDraw.length) {
        grid.innerHTML = '';
        empty.classList.remove('hidden');
      } else {
        empty.classList.add('hidden');
        grid.innerHTML = toDraw.map(cardHTML).join('');
      }
    }
    render(current);
    ...
    search?.addEventListener('input', () => {
      ...
      t = setTimeout(() => {
        const q = (search.value || '').toLowerCase();
        current = q
          ? all.filter(i => nameOf(i).toLowerCase().includes(q))
          : all.slice();
        render(current);
      }, 120);
    });
    ...
    // Sorter change
    const sorter = el.querySelector('#ipSort');
    sorter?.addEventListener('change', () => {
      sortMode = sorter.value || 'latest';
      // Recompute from current search subset
      render(current);
    });
  ...
  async function updateTitle(brandId, titleId, payload) {
    ...
    async function __tm_listMediaAll() {
      ...
      function __tm_mediaAssetId(it) {
        // Generic: if the summary already carries a thumbnailAssetId, use it for ANY mediaType.
        // (Video keeps working; image/audio will start enriching as soon as the API fills this field.)
        return it?.thumbnailAssetId ? String(it.thumbnailAssetId) : null;
      }
    ...
    async function __tm_enrichThumbnailsFromMedia(items) {
      ...
      // Collect any items that already expose thumbnailAssetId (all media types)
      ...
      items.forEach(it => {
        ...
        if (url) it.thumbnailUrl = url; // works for video, image, audio
      });
      ...
      async function worker(){
        while (idx < queue.length){
          ...
          if (full) {
            ...
            if (i >= 0) {
              // 1) Merge full detail into the summary row
              arr[i] = { ...arr[i], ...full };

              // 2) If this is image/audio and we still lack a list thumbnail,
              //    derive it from the thumbnail *assetId* in the full record.
              try {
                const mt = String(full?.mediaType || full?.type || '').toLowerCase();
                const isImg = mt === 'image';
                const isAud = mt === 'audio';

                // Prefer the explicit thumbnail assetId if present
                const thumbAid =
                  (isImg && full?.media?.image?.thumbnail?.assetId) ||
                  (isAud && full?.media?.audio?.thumbnail?.assetId) ||
                  null;

                // Backfill missing catalog fields for the list row
                if (thumbAid) {
                  if (!arr[i].thumbnailAssetId) arr[i].thumbnailAssetId = String(thumbAid);

                  if (!arr[i].thumbnailUrl) {
                    // Use the same preview resolver your pickers & edit form use
                    const pv = await __tm_getPreviewFor(String(thumbAid), { isThumbSlot: true });
                    if (pv?.thumb) arr[i].thumbnailUrl = pv.thumb;
                  }
                }
              } catch (_) {
              ...
              // 3) Re-apply filters/sort so the UI picks up thumbnailUrl as soon as it's ready
...
function tmApplyFiltersAndSort(root){
    ...
    if (!listEl.__tmDeleteBound) {
      listEl.addEventListener('click', async (ev) => {
        ...
        try {
          ...
          // After a delete, force the default sort (Latest → Oldest) and re-apply the pipeline
          try {
            const root = __tm_getRootForFilters ? __tm_getRootForFilters() : document;
            const sortSel = root && root.querySelector ? root.querySelector('#tmSort') : null;
            if (sortSel) sortSel.value = 'latest';  // ensure default is selected

            if (typeof tmApplyFiltersAndSort === 'function') {
              tmApplyFiltersAndSort(root);          // uses cache + current filters/sort
            } else {
              // Fallback: do a quick local "latest" sort and render
              const byDateDesc = (a, b) => {
                const da = new Date(a.updatedAt || a.createdAt || a.releaseDate || 0).getTime();
                const db = new Date(b.updatedAt || b.createdAt || b.releaseDate || 0).getTime();
                return db - da;
              };
              const next = (__titles.cacheByBrand[brandId] || []).slice().sort(byDateDesc);
              renderList(listEl, next);
              annotateListWithIds(listEl, next);
            }
          } catch (e) {
            console.warn('[TitleManager] re-sort after delete failed; rendering fallback', e);
  ...
  function renderEditForm(brandId, titleObj) {
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-3">
              ...
              <input id="mPurchasePrice" type="number" min="4.99" step="0.01"
                    ...
                    class="w-40 bg-gray-800 border border-white/10 rounded px-2 py-1.5 disabled:opacity-40" disabled style="display:none" />
            </div>
                <div id="oneTimeBlock" class="mt-4">
                  <label class="block text-sm font-medium text-white/80">Purchase (One-Time)</label>
                    <div id="oneTimeList" class="mt-2 space-y-2"></div>
                    <p id="oneTimeEmpty" class="hidden text-xs text-white/50 mt-1">
                    No active one-time products available.
                    <a href="/creator/products" class="underline">Manage products</a>
                    </p>
                </div>
                ...
                <div id="rentalBlock" class="mt-4">
                  <label class="block text-sm font-medium text-white/80">Rental</label>
                    <div id="rentalList" class="mt-2 space-y-2"></div>
                      <p id="rentalEmpty" class="hidden text-xs text-white/50 mt-1">
                      No active rental products available.
                    <a href="/creator/products" class="underline">Manage products</a>
                  </p>
                </div>
    ...
    // === Monetization product selector (Edit) ===
    (async () => {
      const formEl =
        (ensureOverlayShell && ensureOverlayShell().querySelector('#tmFormEdit')) ||
        document.querySelector('#tmFormEdit');
      if (!formEl) return;

      // 1) Resolve creator & fetch active creator products
      const creatorSub = await tmGetCreatorSub();
      const products   = await tmFetchActiveProductsForCreator(creatorSub);

      // 2) Brand-scope for THIS edit (prefer the title's brandId)
      const editBrandId = (titleObj && titleObj.brandId) ? titleObj.brandId : (brandId || '');

      const brandProducts = (products || []).filter(p => {
        const pid = p?.brandId || p?.brand || p?.brandID || '';
        return pid === editBrandId;
      });

      // 3) Split lists for radios
      __tmMonetState.oneTimeProducts = brandProducts.filter(p => p.type === 'one_time');
      __tmMonetState.rentalProducts  = brandProducts.filter(p => p.type === 'rental');

      // 4) Preselect saved choices from the Title record
      const preOne =
        titleObj?.pricing?.one_time?.productID ||
        titleObj?.purchases?.one_time?.productID ||
        null;

      const preRen =
        titleObj?.pricing?.rental?.productID ||
        titleObj?.rentals?.rental?.productID ||
        null;


      tmRenderMonetSelectors(formEl, {
        selectedOneTime: preOne,
        selectedRental:  preRen,
        warnIfMissing:   true
      });

      // 5) Toggle enable/disable to match availability
      tmSyncTogglesWithAvailability(formEl);

      // Optional: quick debug handle (you can remove later)
      // window.__monetDebug = { editBrandId, products, brandProducts };
    })();
  }
  ...
  function hydrateEditFormFromTitle(t) {
    ...
    (async () => {
      const el     = ensureOverlayShell();
      const formEl = el.querySelector('#tmFormEdit');
      if (!formEl) return;

      // 0) Hide legacy price inputs
      ['#mPurchasePrice', '#mRentalPrice', '#mRentalWindow'].forEach(sel => {
        const n = formEl.querySelector(sel);
        if (n) {
          n.value = '';
          n.disabled = true;
          n.style.display = 'none';
          n.closest('label')?.classList.add('opacity-50','pointer-events-none');
        }
      });

      // 1) Resolve creatorSub
      const creatorSub = await tmGetCreatorSub();

      // 2) Preselect from existing Title record
      __tmMonetState.selectedOneTimeProductId =
        t?.pricing?.one_time?.productID ||
        t?.purchases?.one_time?.productID ||
        null;

      __tmMonetState.selectedRentalProductId  =
        t?.pricing?.rental?.productID ||
        t?.rentals?.rental?.productID ||
        null;

      // 3) Fetch + render radios with preselects
      const products = await tmFetchActiveProductsForCreator(creatorSub);
      const editBrandId = t?.brandId || (window.__titles?.openForBrand) || '';

      const brandProducts = products.filter(p => p.brandId === editBrandId);

      __tmMonetState.oneTimeProducts = brandProducts.filter(p => p.type === 'one_time');
      __tmMonetState.rentalProducts  = brandProducts.filter(p => p.type === 'rental');

      tmRenderMonetSelectors(formEl, {
        selectedOneTime: __tmMonetState.selectedOneTimeProductId,
        selectedRental:  __tmMonetState.selectedRentalProductId,
        warnIfMissing:   true
      });
      tmSyncTogglesWithAvailability(formEl);


      // 4) Keep pricing.type in sync (same behavior as Create)
      formEl.addEventListener('change', (e) => {
        const x = e.target;
        if (!(x instanceof HTMLElement)) return;

        if (x.matches('input[type="radio"][name="tmOneTime"]')) {
          const chk = formEl.querySelector('#mPurchase');
          if (chk) chk.checked = true;
        }
        if (x.matches('input[type="radio"][name="tmRental"]')) {
          const chk = formEl.querySelector('#mRental');
          if (chk) chk.checked = true;
        }
        if (x.matches('#mPurchase') && !x.checked) {
          formEl.querySelectorAll('input[type="radio"][name="tmOneTime"]').forEach(n => { n.checked = false; });
        }
        if (x.matches('#mRental') && !x.checked) {
          formEl.querySelectorAll('input[type="radio"][name="tmRental"]').forEach(n => { n.checked = false; });
        }
      }, { passive: true });
    })();
    ...
    if (btnSave && !btnSave.__tmBound) {
      btnSave.addEventListener('click', async (e) => {
        ...
        try {
          ...
          // [ADD #1] form element for the edit overlay
          const formEl = ensureOverlayShell().querySelector('#tmFormEdit');

          // [ADD #2] keep your purchases/rentals block in sync (existing helper)
          tmAttachMonetizationToPayload(payload, formEl);

          // [ADD #3] NEW: mirror selected product IDs into pricing.one_time/rental.productID
          tmAttachProductPointersToPricing(payload, formEl);

          // ⬇️ existing PATCH — leave as-is
          ...
          try {
            await tmTriggerFeedBuild(brandId, titleId);
          } catch {}
...
function buildPayloadFromOverlayForm() {
  ...
  // NEW: read selected product radios from the form
  const oneProdSel = (form.querySelector('input[type="radio"][name="tmOneTime"]:checked')?.value || null);
  const renProdSel = (form.querySelector('input[type="radio"][name="tmRental"]:checked')?.value || null);
  ...
  if (mFree) {
    // Free is exclusive; OK
  } else {
    // Consider product radios as “picked” monetization too
    const picked = [mSub, mPur, mRen].some(Boolean) || !!oneProdSel || !!renProdSel;
    if (!picked) {
      throw new Error('Select at least one monetization (Subscription/Purchase/Rental), pick a product, or choose Free.');
    }

    // If a product is selected, skip legacy price validation (product defines price)
    if (mPur && !oneProdSel) {
      if (!(purPrice >= 4.99)) throw new Error('Purchase price must be at least $4.99.');
    }
    if (mRen && !renProdSel) {
  ...
  if (mFree) {
    ...
    if (oneProdSel || mPur) pricingType.push('purchase');
    if (renProdSel || mRen) pricingType.push('rental');
  }

  const pricing = { type: pricingType };
  // Only write legacy price/window when NO product was selected
  if (pricingType.includes('purchase') && !oneProdSel && (typeof purPrice === 'number')) {
    pricing.purchase = { price: Number(purPrice.toFixed(2)) };
  }
  if (pricingType.includes('rental') && !renProdSel && (typeof renPrice === 'number')) {
    const wh = Number(renWin);
    pricing.rental = { price: Number(renPrice.toFixed(2)), ...(Number.isFinite(wh) ? { windowHours: wh } : {}) };
  }
  ...
  // ✅ Merge one_time / rental product selections into payload (AFTER payload exists)
  {
    const overlay2 = (ensureOverlayShell && ensureOverlayShell()) || document;
    const formEl  = overlay2.querySelector('#tmFormEdit') || overlay2.querySelector('#tmForm'); // prefer Edit, else Create
    if (formEl) tmAttachMonetizationToPayload(payload, formEl);
  }
  ...
  function renderCreateForm(brandId) {
            ...
            <div>
              ...
              <!-- Monetization (new) -->
                <div class="space-y-2">
                  ...
                  <!-- ── Monetization: Product selectors (One-Time & Rental) ── -->
                  <div id="oneTimeBlock" class="mt-4">
                          <label class="block text-sm font-medium text-white/80">Purchase (One-Time)</label>
                          <div id="oneTimeList" class="mt-2 space-y-2"></div>
                          <p id="oneTimeEmpty" class="hidden text-xs text-white/50 mt-1">
    ...
    // === Monetization product selector (Create) ===
    (async () => {
      const overlay = ensureOverlayShell();
      const formEl  = overlay.querySelector('#tmForm');
      if (!formEl) return;

      // 0) Hide the legacy price inputs so they don't conflict with product radios
      ['#mPurchasePrice', '#mRentalPrice', '#mRentalWindow'].forEach(sel => {
        const n = formEl.querySelector(sel);
        if (n) {
          n.value = '';
      ...
      // 1) Resolve creatorSub (uses your helper)
      const creatorSub = await tmGetCreatorSub();

      // 2) Fetch active products and render radios (no preselect on CREATE)
      const products = await tmFetchActiveProductsForCreator(creatorSub);
      ...
      tmRenderMonetSelectors(formEl, { selectedOneTime: null, selectedRental: null });
      tmSyncTogglesWithAvailability(formEl);

      // 3) Keep pricing.type in sync:
      //    - When a radio is picked, auto-check the matching checkbox
      //    - When a checkbox is turned OFF, clear the radios for that group
      formEl.addEventListener('change', (e) => {
        const t = e.target;
        if (!(t instanceof HTMLElement)) return;

        // Radio -> check the corresponding checkbox
        if (t.matches('input[type="radio"][name="tmOneTime"]')) {
          const chk = formEl.querySelector('#mPurchase');
          if (chk) chk.checked = true;
        }
        if (t.matches('input[type="radio"][name="tmRental"]')) {
          const chk = formEl.querySelector('#mRental');
          if (chk) chk.checked = true;
        }

        // Checkbox off -> clear that group’s radios
        if (t.matches('#mPurchase') && !t.checked) {
          formEl.querySelectorAll('input[type="radio"][name="tmOneTime"]').forEach(n => { n.checked = false; });
        }
        if (t.matches('#mRental') && !t.checked) {
          formEl.querySelectorAll('input[type="radio"][name="tmRental"]').forEach(n => { n.checked = false; });
        }
      }, { passive: true });
    })();
  ...
  function bindCreateForm(brandId) {
    ...
    if (btnSave && !btnSave.__tmBound) {
      ...
      try {
        ...
        // [ADD #1] get the form element for this overlay
        const formEl = ensureOverlayShell().querySelector('#tmForm');

        // [ADD #2] keep your purchases/rentals block in sync (existing helper you already have)
        tmAttachMonetizationToPayload(payload, formEl);

        // [ADD #3] NEW: mirror selected product IDs into pricing.one_time/rental.productID
        tmAttachProductPointersToPricing(payload, formEl);

        // ✅ keep the existing API call exactly as-is
        const resp = await createTitle(brandId, payload);
          ...
          try {
            await tmTriggerFeedBuild(brandId, resp?.titleId);
          } catch {}
    ...
    function buildPayloadFromForm() {
      ...
      if (mFree) {
        ...
        // Count either a checkbox OR a selected product as "picked"
        const picked = [mSub, mPur, mRen].some(Boolean) || !!oneProdSel || !!renProdSel;
        if (!picked) {
          throw new Error('Select at least one monetization (Subscription/Purchase/Rental), pick a product, or choose Free.');
        }

        // If a product is selected, skip old price validation for that type
        if (mPur && !oneProdSel) {
          if (!(purPrice >= 4.99)) throw new Error('Purchase price must be at least $4.99.');
        }
        if (mRen && !renProdSel) {
      ...
      {
        const overlay = (ensureOverlayShell && ensureOverlayShell()) || document;
        const formEl  = overlay.querySelector('#tmFormEdit') || overlay.querySelector('#tmForm');
        if (formEl) tmAttachMonetizationToPayload(payload, formEl);
      }
...
function __tm_imageLandscapeFrom(mu) {
  // 🔴 Exact requirement first: mediaUrl > artwork > primary > landscape
  const exact = mu?.artwork?.primary?.landscape;
  if (exact) return exact;

  // Common shapes under image.primary.artwork
  const ipr = mu?.image?.primary || mu?.primary || {};
  const awp = ipr?.artwork || {};

  // Also consider top-level artwork.primary and other safe fallbacks
  const awTop = mu?.artwork || {};

  return (
    awp?.landscape ||
    awTop?.primary?.landscape ||
    awTop?.landscape ||
    awTop?.['16x9'] ||
    awTop?.standard ||
    awTop?.portrait ||
  ...
  function renderMediaBlocks() {
    ...
    host.querySelectorAll('[data-pick]').forEach(btn => {
      ...
      btn.addEventListener('click', async (e) => {
        ...
        // audio-only tweak: treat Audio Primary like a thumbnail pick (we only need assetId)
        const idOnly = (/Thumb$/.test(slot) || slot === 'audPrimary');
...
async function pickAssetFromLibrary({ filter = 'image', brandId = null, idOnly = false } = {}) {
  ...
  function isProcessed(it) {
    const s = String(it.status || it.processingStatus || '').toLowerCase();
    return s === 'processed';
  }
  ...
  function filterItems(items){
    return items.filter(it => {
      ...
      if (!t) return false;
      const typeOk =
        wanted === 'image' ? t.startsWith('image') :
        wanted === 'video' ? t.startsWith('video') :
        wanted === 'audio' ? t.startsWith('audio') :
        true;

      // 2) processed-only
      return typeOk && isProcessed(it);
    });
  }
...
function buildUrlIndex(items){
  ...
  items.forEach(it => {
    ...
    const aud  = mu.audio?.primary || {}; // ← NEW
    ...
    const candidates = [
      ...
      // ⬇️ new: let audio URLs resolve back to their assets just like video/image do
      mu?.audio?.primary?.url,
      mu?.audio?.primary?.src,
      mu?.audio?.primary?.href,
      // ⬆️ end audio additions
  ...
  function buildIdIndex(items){
    const m = new Map();
    items.forEach(it => {
      const id = it.assetId || it.assetID || it.id || it._id || it.mediaId || it.fileId || it.key || it.uuid || it.audioId;
  ...
  function normalizeFromItem(it){
    ...
    const mu = it.mediaUrl || it.media_url || it.mediaURL || {};
    ...
    // AUDIO primary URL (common shapes)
    const apr = mu.audio?.primary || {};
    const urlAudio = apr.url || apr.src || apr.href || '';

    // IMAGE shapes/preview (unchanged)
    const ipr = mu.image?.primary || mu.primary || {};
    ...
    // Choose a URL like video/image do:
    const chosenUrl = isVideo ? urlVideo : (isAudio ? (urlAudio || imagePreview) : imagePreview);

    // Choose a preview thumb:
    // - video: use video thumbnails
    // - audio: reuse same thumbnail logic as images (artwork.primary.landscape)
    let thumb = '';
    if (isVideo) thumb = videoThumb || '';
    else {
      // __tm_thumbFrom matches your existing thumbnail selection heuristic
      thumb = __tm_thumbFrom(mu) || imagePreview;
    }
  ...
  let   items = filterItems(all);

  // Default sort: Latest → Oldest (use updatedAt, else createdAt, else mediaUrl timestamps)
  function dateOf(it){
    const mu = it.mediaUrl || it.media_url || {};
    const candidates = [
      it.updatedAt, it.modifiedAt, it.lastModified,
  ...
  items.sort((a,b) => dateOf(b) - dateOf(a)); // Latest → Oldest
  ...
  function resolveSelection(sel){
    ...
    // Accept assetId even if url is missing — primaries only need assetId
    if (cand.assetId) {
      const out = { assetId: String(cand.assetId) };
      if (cand.url) out.url = String(cand.url);
      return out;
    }

    const idOnlyValue = cand.assetId || cand.assetID || cand.id || cand._id || cand.mediaId || cand.fileId || cand.key || cand.uuid || cand.audioId;
    if (idOnlyValue) {
